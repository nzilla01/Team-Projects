...
namespace FinTrack.Server.Controllers
{
        ...
        public async Task<ActionResult<IEnumerable<CategoryDto>>> GetAll()
        {
            var cats = await _db.Categories
                .Select(c => new CategoryDto
                {
                    ...
                    Color = c.Color, Type = c.Type.ToString()
                }).ToListAsync();
            return Ok(cats);
        }
    }
    ...
    public class SavingsGoalsController : ControllerBase
    {
        ...
        public async Task<ActionResult<SavingsGoalDto>> Create([FromBody] CreateSavingsGoalDto dto)
        {
            ...
            {
                UserId = UserId, Title = dto.Title, Description = dto.Description,
                TargetAmount = dto.TargetAmount, TargetDate = dto.TargetDate
            };
    ...
    public class DashboardController : ControllerBase
    {
        ...
        {
            var transactions = await _db.Transactions.Include(t => t.Category)
            ...
            var recent = transactions.OrderByDescending(t => t.Date).Take(5)
                ...
                {
                    Id = t.Id, CategoryId = t.CategoryId,
                    CategoryName = t.Category?.Name ?? "", CategoryIcon = t.Category?.Icon ?? "",
                    CategoryColor = t.Category?.Color ?? "", Amount = t.Amount,
                    Description = t.Description, Date = t.Date, Type = t.Type.ToString()
                }).ToList();

            var spendingByCategory = transactions.Where(t => t.Type == TransactionType.Expense)
                .GroupBy(t => t.Category)
                .Select(g => new CategorySpendDto
                {
                    CategoryName = g.Key?.Name ?? "Other", CategoryColor = g.Key?.Color ?? "#6B7280",
            ...
            return Ok(new DashboardDto
            {
                ...
                RecentTransactions = recent, SpendingByCategory = spendingByCategory
            });
        }
    }
}
